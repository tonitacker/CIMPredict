...
def get_base_path():
    if getattr(sys, 'frozen', False):
        # Wenn das Skript als App ausgeführt wird
        base_path = os.path.dirname(sys.executable)
        base_path_exe = "True"
       # raise Exception(base_path)
    else:
...
def configure_r_environment():
    # Pfad zur eingebetteten R-Version relativ zum Skript
    base_dir = os.path.dirname(os.path.abspath(__file__))
    r_dir = os.path.join(base_dir, "portableR", "Resources")

    # Sicherstellen, dass die portable R-Version existiert
    if not os.path.exists(r_dir):
        raise EnvironmentError(f"R directory not found: {r_dir}")

    # Setze Umgebungsvariablen für R
    os.environ['R_HOME'] = r_dir
    os.environ['R_LIBS_USER'] = os.path.join(r_dir, "library")

    print(f"Configured R_HOME: {os.environ['R_HOME']}")
    print(f"Configured R_LIBS_USER: {os.environ['R_LIBS_USER']}")
...
if(base_path_exe == "True"):
    resource_path = get_resource_path(currentwd)
else:
    resource_path = currentwd
print(currentwd)
#dll_dir = os.path.normpath(os.path.join(currentwd, "portableR","4.4.4_2","bin", "x64"))
